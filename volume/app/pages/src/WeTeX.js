...
function renderAllWeTeXMath() {
    ...
    for (var i = 0; i < mds.length; i++) {
        mdsi = mds[i];
        mdsi.style.backgroundColor = "magenta";
...
function replaceMathArrowWithTeX(item) {
    /*
    var para = document.querySelectorAll("svg.xleftarrow");
    for (var i in para) {
    item.outerHTML = "\\xyzleftarrow{" + c[0].innerHTML + "}";
    //console.log("L Value is:",para[i]);
    }
    */
    /*
    var statusHistory = document.getElementById('longaccents');

    if(statusHistory.classList.contains('xrightarrow')) {
        item.outerHTML = "\\zyxrightarrow{" + c[1].innerHTML + "}";
    }

    if(statusHistory.classList.contains('xleftarrow')) {
        item.outerHTML = "\\xyzleftarrow{" + c[0].innerHTML + "}";
    }
    */
}
...
function longarrows(nd) {
    all = nd.querySelectorAll(".xleftarrow,.xrightarrow");
    for (var j = 0; j < all.length; j++) {
        var allj = all[j];
        addAttributeToNode(allj, "clickon", "addFocus(this);");
    }
}


function katexHtml2latex(m1) {
    runQueryAll(m1, ".strut,.pstrut0,.vlist-s,.katex-mathml", removeTag);
    runQueryAll(m1, ".mrel.x-arrow", longarrows);
    runQueryAll(m1, "svg,.mrel.x-arrow", removeTag);
...
function reloadEditedKaTeX() {
    ...
    if (m1 != null) {
        ...
        t1 = document.getElementById(texid);
        mml1 = document.getElementById(mmlid);
        mh1 = parseHtmlForTeX(m1.innerHTML); //TeX extract from mathml
        t1.value = mh1;
        WeTeXDisplay = true;
...
function setCursorPositionOnBox(m1) {
    var tags = m1.querySelectorAll(".mord.amsrm,.mord.cjk_fallback,.katex-html"); //katex-html added bala
    L = tags.length;
    tag = null;
    for (var i = 0; i < L; i++) {
