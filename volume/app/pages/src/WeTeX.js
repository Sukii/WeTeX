...
function replaceMathAccentWithTeX(item) {
    ...
    switch (s) {
        case "\^":
            ...
            break;
        case ".mathbb":
            item.outerHTML = "\\mathBB{" + c[0].innerHTML + "}";
            break;
        case "ˉ":
...
function katexHtml2latex(m1) {
    ...
    runQueryAll(m1, "span.mord", replaceMathbbWithTeX)
...
function setCursorPositionOnBox(m1) {
    var tags = m1.querySelectorAll(".mord.amsrm,.mord.cjk_fallback"); //katex-html added bala
    L = tags.length;
    tag = null;
    for (var i = 0; i < L; i++) {
