...
function replaceMathitWithTeX(item) {
    item.outerHTML = "\\mathit{" + item.innerHTML + "}";
}
...
function replaceboldsymWithTeX(item) {
	item.outerHTML = "\\bm{" + item.innerHTML + "}";
}
...
function replaceMtableWithTeX(item) {
	...
	for(var j=0; j<M; j++) {
		for(var i=0; i<L; i++) {
			...
			if(i != L-1) { mtblh = mtblh + "&amp;"; }
		}
			if(j != M-1) { mtblh = mtblh + "\\\\ "; }	
	}
...
function katexHtml2latex(m1) {
	...
	runQueryAll(m1, ".mord.mainit", replaceMathitWithTeX);
...
function bindArrowKeys(lstn) {
    lstn.sequence_combo("right", function () {
        console.log("pressed right arrow");
    });
    lstn.sequence_combo("left", function () {
